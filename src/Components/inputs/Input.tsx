...
const Input: React.FC<InputProps> = ({ id, type, label, errors, register, required }) => {
    return (
        <div className='w-full sm:min-w-96 min-w-80 relative text-slate-800'>
            <input autoComplete='off' placeholder='' {...register(id, { required })} className={`peer w-full px-4   py-4 outline-none bg-gray-100 font-light border-2 rounded-md transition disabled:opacity-70 disabled:cursor-not-allowed ${errors[id] ? 'border-rose-400 focus:border-rose-400' : 'focus:border-slate-400 border-slate-300'} `} type={type} id={id} />
